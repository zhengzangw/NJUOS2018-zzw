...
// ========== LIST ===========
#define Lognode(tag, node) \
    Log(tag "node: start=%p, end=%p", node->start, node->end)
// ========== SPINLOCK =======
...
#define Logcontext(context) \
    Log("id=%d, run=%d, sleep=%d, name=%s", context->id, context->run, context->sleep, context->name)
// ========== LIST ===========
#define Logsem(sem) \
