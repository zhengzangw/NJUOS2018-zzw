...
static void dev_init() {
  ...
  kmt->create(pmm->alloc(sizeof(task_t)), "shell-task-1", shell_task, "/dev/tty1");
  kmt->create(pmm->alloc(sizeof(task_t)), "shell-task-2", shell_task, "/dev/tty2");
  kmt->create(pmm->alloc(sizeof(task_t)), "echo-task-1", echo_task, "tty3");
  kmt->create(pmm->alloc(sizeof(task_t)), "echo-task-2", echo_task, "tty4");
}
